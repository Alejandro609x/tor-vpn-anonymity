...
def instalar_tor_browser():
    ...
    subprocess.run(["apt", "install", "-y", "torbrowser-launcher"])
...
def abrir_navegador(nav, puerto):
    ...
    # CONTROL DE INSTANCIAS DE FIREFOX
    if "firefox" in nav:
        try:
            ...
            if check_proc.returncode == 0:
                ...
                respuesta = input(f"{Y}[?] ¿Deseas cerrar las ventanas actuales de Firefox? (s/n): {X}").lower()
                
                if respuesta == 's':
                    print(f"{R}[+] Forzando el cierre total de Firefox...{X}")
                    subprocess.run(["pkill", "-9", "-f", "firefox"])
                    time.sleep(3)
                else:
                    print(f"{Y}[!] Continuando. Nota: La nueva ventana podría NO usar la red anónima.{X}")
        except:
            pass
    ...
    try:
        if "firefox" in nav:
            perfil_temp = os.path.join(home, ".mozilla/firefox/jefazo.default")
            ...
            subprocess.run(["chown", "-R", f"{usuario_real}:{usuario_real}", perfil_temp])

            prefs = os.path.join(perfil_temp, "user.js")
...
""")
            ...
            # CAMBIO ADICIONAL: Agregado --log-level=3 para desactivar los warnings internos/D-Bus de Chromium
            subprocess.Popen([
                "sudo", "-u", usuario_real, nav,
        ...
        print(f"{R}[!] Error al lanzar el navegador:{X} {e}")
